...
namespace woocommerce_events\Engine;

use woocommerce_events\Engine;

/**
 * woocommerce_events Initializer
 */
class Initialize {
	...
	private function load_classes() {
		$this->classes = \apply_filters( 'woocommerce_events_classes_to_execute', $this->classes );

		foreach ( $this->classes as $class ) {
			try {
				$this->initialize_plugin_class( $class );
			} catch ( \Throwable $err ) {
				\do_action( 'woocommerce_events_initialize_failed', $err );
	...
	private function get_classes( string $namespacetofind ) {
		...
		$namespacetofind = 'woocommerce_events\\' . $namespacetofind;

		// In case composer has autoload optimized
		if ( isset( $classmap[ 'woocommerce_events\\Engine\\Initialize' ] ) ) {
		...
		if ( isset( $prefix[ $namespacetofind ] ) ) {
			...
			if ( !WP_DEBUG ) {
				\wp_die( \esc_html__( 'woocommerce-events is on production environment with missing `composer dumpautoload -o` that will improve the performance on autoloading itself.', W_TEXTDOMAIN ) );
			}
